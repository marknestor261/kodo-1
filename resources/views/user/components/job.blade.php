        ...
        <div class="card-body p-0">
            <div class="options-inv">
                ...
                <div class="dropdown-menu dropdown-menu-start">
                    @if ($x->saved)
                        <a class="dropdown-item" href={{ route("user.unsave.job",[ 'program_id' => $x->id]) }}><i class="fa fa-bookmark me-2"></i> Saved</a>
                    @else
                        <a class="dropdown-item" href={{ route("user.save.job",[ 'program_id' => $x->id]) }}><i class="fe fe-bookmark me-2"></i> Save</a>
                    @endif

                    @if($x->banned)
                        <a class="dropdown-item" href={{ route("user.unban.job",[ 'program_id' => $x->id]) }}><i class="fe fe-slash me-2"></i> Banned</a>
                    @else
                        <a class="dropdown-item" href={{ route("user.ban.job",[ 'program_id' => $x->id]) }}><i class="fe fe-slash me-2"></i> Ban</a>
                    @endif
                </div>
            </div>
            <div class="d-flex align-items-center justify-content-between py-5 px-6 mt-2">
                ...
                <div>
                    @if(auth()->user()->is_paid)
                        <a href={{ route('user.job', ['program_id' => $x->id]) }} class="btn btn-info">Apply</a>
                    @else
